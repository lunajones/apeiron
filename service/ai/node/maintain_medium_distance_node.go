...
func (n *MaintainMediumDistanceNode) Tick(c *creature.Creature, ctx interface{}) interface{} {
	...
	if grid == nil {
		log.Printf("[AI] [%s (%s)] grid indisponível para MaintainMediumDistanceNode", c.Handle.String(), c.PrimaryType)
		return core.StatusFailure
	}
	...
	if dist < 4.0 {
		...
		newPos := position.FromGlobal(
			...
			c.GetPosition().FastGlobalY()+dir.Y*2.0,
			c.GetPosition().Z+dir.Z*2.0,
		)

		c.MoveCtrl.SetTarget(newPos, c.RunSpeed, stopAt)
		...
		c.MoveCtrl.SetTarget(target.GetPosition(), c.RunSpeed, stopAt)
		c.SetAction(consts.ActionRun)
		c.MoveCtrl.Update(c, 0.016, grid)
		return core.StatusRunning
	}
