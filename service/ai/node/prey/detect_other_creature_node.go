...
func (n *DetectOtherCreatureNode) Tick(c *creature.Creature, ctx interface{}) interface{} {
	...
	if !ok {
		log.Printf("[AI] [%s (%s)] contexto inválido para DetectOtherCreatureNode (prey)", c.Handle.ID, c.PrimaryType)
	...
	for _, other := range svcCtx.GetServiceCreatures(c.Position, c.DetectionRadius) {
		...
		log.Printf("[AI] [%s (%s)] avaliando %s (%s), distância: %.2f",
			c.Handle.ID, c.PrimaryType, other.Handle.ID, other.PrimaryType, dist)
	...
	hunger := c.GetNeedValue(consts.NeedHunger)
	if hunger > 40 && bestTarget.HasTag(consts.TagPrey) {
