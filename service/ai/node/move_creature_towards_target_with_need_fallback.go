...
func (n *MoveTowardsTargetWithNeedFallbackNode) Tick(c *creature.Creature, ctx interface{}) interface{} {
	...
	if grid == nil {
		log.Printf("[MOVE WITH NEED FALLBACK] [%s (%s)] grid indisponível", c.GetHandle().ID, c.PrimaryType)
		return core.StatusFailure
	}
	...
	c.MoveCtrl.SetTarget(target.GetPosition(), c.RunSpeed, n.StopDistance)
	reached := c.MoveCtrl.Update(c, 0.016, grid)

	if reached {
		log.Printf("[MOVE WITH NEED FALLBACK] [%s (%s)] alcançou alvo após mover (%.2f ≤ %.2f)",
			c.GetHandle().ID, c.PrimaryType, dist, n.StopDistance)
		return core.StatusSuccess
	}

	c.SetAction(consts.ActionRun)
	log.Printf("[MOVE WITH NEED FALLBACK] [%s (%s)] movendo em direção ao alvo (%.2f > %.2f)",
		c.GetHandle().ID, c.PrimaryType, dist, n.StopDistance)
